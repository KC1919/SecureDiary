...
noteRouter.post("/imageUpload", upload.single('uploadImage'), async (req, res) => {
    ...
    // console.log(buffer);
    // Encrypt
    // let ciphertext = encrypt(buff, "kunal");
...
noteRouter.get("/textNotes",verify, async (req, res) => {
    ...
    const textNotes=await Note.find({user:req.userId},{text:1});
    ...
    res.render("textfiles",{"notes":notes});
})
...
noteRouter.post("/decrypt",verify,async(req,res)=>{
    ...
    let user=await User.find({user:req.userId});

    console.log(user);

    if(user){
        const fetchedCipher=await Note.find({user:req.userId},{"text":{$elemMatch:{"_id":noteId}}})
        ...
        return res.json({message:"Decrypted text",data:plainText});
    }
    else{
